...
ポテンショメータはアナログデバイスで、2つの異なる方向に回すことができます。

ポテンショメータの中央のピンをアナログピンGP28に接続します。Raspberry Pi Pico Wには多チャンネルの16ビットアナログ-デジタルコンバータが搭載されています。これは、入力電圧（0から動作電圧の3.3Vの間）を整数値（0から1023の間）にマッピングすることを意味します。そのため、GP28の値は0から1023の範囲になります。
...
こうすることで、ポテンショメータを回転させると、LEDの明るさが同時に変化することがわかります。
